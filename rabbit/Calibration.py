...
from rabbit.python.ndvi_to_K_map_beta import ndvi_to_K_map
...
# Donana 
obs_df = pd.read_csv(path.join(getcwd(), "rabbit", "data" , "original_data", "Rabbit_donana_KAI_PacoCarro", "KAI_Rabbit_Night_2024_v1.csv"), skipinitialspace=True)
...
# get R functions
r = robjects.r
r['source'](path.join(getcwd(), "rabbit", 'R', 'function_rabbit_distances.R'))

get_sim_data = robjects.globalenv['get_sim_data']
csvToRaster = robjects.globalenv['csvToRaster']

hab_file = path.join(getcwd(), "rabbit", "data", "GIS_maps", "Rabbit_HabitatMap_500_Donana_Fordham_2013.asc")
transectsD = path.join(getcwd(), "rabbit", "data", 'original_data', "Rabbit_donana_KAI_PacoCarro", "Transect_oryctolagus.kml")
ndvi_map = path.join(getcwd(), "rabbit", "data", "model_input", "maps", "ndvi_monthly_maps")
flood_map = path.join(getcwd(), "rabbit", "data", "model_input", "maps", "LAST_floodmaps")
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for r in range(n_repeats):
                ...
                cmd = [path.join("rabbit", "Pascal_program","programs","Rabbit_solo_model.exe"), 
                       path.join("rabbit", "data", "model_input", "Parameters_historic_calibration.txt"), 
                       sim_dir,
                       str(parameters['dens_opt']), 
                       str(parameters['R_lambda']), 
                       str(parameters['R_sigma']),
                       path.join("rabbit", "data", "model_input", "maps", "Rabbit_HabitatMap_500_Donana_Fordham_2013.txt"),
                       hab_folder
                       ] 
...
best_estimates_df.to_csv(path.join("rabbit", 'results', 'parameter_estimates.csv'))
...
fig.tight_layout()
plt.savefig(path.join("rabbit", 'results', 'parameter_abc_diagnostics.png'))
...
df_final.to_csv(path.join("rabbit", 'results', 'posterior_particles.csv'), index=False)
np.save(path.join("rabbit", 'results', 'posterior_weights.npy'), w_final)
...
plt.tight_layout()
plt.savefig(path.join("rabbit", 'results', 'ndvi_beta_distribution.png'))
plt.show()


# Run historical simulations and plot -----------------------------------------------------

import rabbit.python.run_history_sim

rabbit.python.run_history_sim.main(N = 5)

# Plot results
robjects.r.assign("run_files", "results/history_summary")
robjects.r.source('rabbit/R/plot_results.R')
