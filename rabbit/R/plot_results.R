...
if(!exists("run_files")) {
  run_files = "results/history_summary/"
} 

map_files <- list.files(run_files, pattern = ".csv$", full.names = T)
...
library(magick)
dir.create(file.path(run_files, "frames"), showWarnings = FALSE)

for (i in seq_along(plots)) {
  ggsave(
    filename = file.path(run_files, "frames", sprintf("frame_%02d.png", i)),
    plot = plots[[i]],
    width = 6,
    height = 4,
    dpi = 150
...
gif <- image_read(list.files(file.path(run_files, "frames"), full.names = TRUE)) |>
  image_animate(fps = 1)  # 1 second per plot

image_write(gif, paste0(run_files, "_plots.gif"))
...
ggsave(paste(run_files, "KAI_comparison.png"), p, width = 8, height = 4)
