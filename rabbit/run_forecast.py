    ...
    for idx, params in sampled_params.iterrows():
        ...
        for rep in range(0, replicates):
            ...
            cmd = [path.join("Pascal_program","programs","Rabbit_solo_model.exe"), 
                   path.join("data", "model_input", "Parameters_future_simulation.txt"), 
                   sim_folder,
                   str(dens_opt), 
                   str(R_lambda), 
                   str(R_sigma),
                   path.join(hab_folder, '2022_01.txt'),
                   hab_folder
                   ] 
            mod = subprocess.run(cmd, capture_output=True, text=True, timeout=500)
    
    summary_maps.main(tmpdir, path.join('results', 'forecasts_summary'))
