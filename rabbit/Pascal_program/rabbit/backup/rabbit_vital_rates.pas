  ...
  begin
    ...
    if (RabbitPopulationSize > 1) then
      ...
      begin
        ...
        if (Rabbit^.Age > juv_age) then
        begin
        rep_p := 0;

        if (Rabbit^.Age < sub_adult_age) then
        begin
           rep_p:= exp(r_int - r_dens_effect * Kdensity) /
           (1 + exp(r_int - r_dens_effect * Kdensity)) ;
        end
        else if Rabbit^.Age < adult_age then
        begin
          rep_p:= exp(r_int + r_second_effect - r_dens_effect * Kdensity) /
           (1 + exp(r_int + r_second_effect - r_dens_effect * Kdensity));
        end
        else
        begin
          rep_p:= exp(r_int + r_later_effect - r_dens_effect * Kdensity) /
           (1 + exp(r_int + r_later_effect - r_dens_effect * Kdensity));
        end;
...
begin
  {Calculate current survival probabilities as a function of density}
        ...
        surv_nb:=exp(ln(1-MortP_at_month_old));

        surv_juv:=(exp(s_int - s_extra_juv -s_dens_effect*Kdensity)/
                 (1+exp(s_int - s_extra_juv -s_dens_effect*Kdensity)))*
                 (1-exp(-s_food_effect/(nbadmonths+0.001)));

        surv_adult:=(exp(s_int-s_dens_effect*Kdensity)/
                 (1+exp(s_int-s_dens_effect*Kdensity)))*
                 (1-exp(-s_food_effect/(nbadmonths+0.001)));
  ...
  begin
    ...
    begin
      ...
      else if Rabbit^.age <= sub_adult_age then surv_p := surv_juv
      else surv_p := surv_adult;

      {Determine fate of Rabbits}
      die := False;
      if Rabbit^.age >= max_age then die := True
      else
    ...
    begin
     ...
     begin
      ...
      begin
       ...
       p_not_stand[c] := exp(-lambda * ((dens) - dens_opt)**2) / ((1 + disp_dist[c])**sigma);
      end;
     end;
