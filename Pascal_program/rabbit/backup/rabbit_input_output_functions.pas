...
//reset(filename);
 ...
 FileContent.LoadFromFile(breedname);
 ...
 for Line in FileContent do
    begin
      ...
      if Parts[0] = 'col' then Continue;

      // Parse the indices
      ColX := StrToInt(Parts[0]);
      RowY := StrToInt(Parts[1]);

      SetLength(BreedingMonths_xy[ColX,RowY], Length(Parts));

      for i := 2 to High(Parts) do
      begin
        value := StrToInt(Parts[i]);
 ...
 FileContent := TStringList.Create;
 FileContent.LoadFromFile(dryname);
 ...
 for Line in FileContent do
    begin
      ...
      SetLength(DryMonths_xy[ColX,RowY], Length(Parts) - 1);
